...
case "$group" in
  ...
  ch05)           file="src/ch05.py";            mode=import-check ;;
  ch06)           file="src/ch06.py";            mode=import-check ;;
  ch07)           file="src/ch07.py";            mode=execute ;;
  ch09)           file="src/ch09.py";            mode=import-check ;;
  # ch10 はリモート取得コードの exec を読者が手動でアンコメントする設計のため非対話実行できない
  ch10)           file="src/ch10.py";            mode=import-check ;;
  # ch11-bayesian は SVI 学習が CPU で数時間かかるため import 検証のみ
  ch11-bayesian)  file="src/ch11_bayesian.py";   mode=import-check ;;
  ch11-dowhy)     file="src/ch11_dowhy.py";      mode=execute ;;
  # ch12 は input() による対話確認を含むため非対話実行できない
  ch12)           file="src/ch12.py";            mode=import-check ;;
  ch13)           file="src/ch13.py";            mode=import-check ;;
